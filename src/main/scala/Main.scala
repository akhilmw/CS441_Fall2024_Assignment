...
import utils.Vectorization.convertToIndArrays
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    // Your input text
    val text = "This is a sample text message This is a sample text message This is a sample text message This is a sample text message"
    ...
    val inputOutputPairs = BytePairUtils.createInputOutputPairs(decoded, windowSize, stride)
    ...
    val vocabSize = BytePairUtils.getMaxVocabSize(decoded)
...
//    // Verify shapes
//    val activations = model.feedForward(inputFeatures, false)
//    activations.foreach { case (layerName, activation) =>
//      println(s"Layer: $layerName, Activation Shape: ${activation.shape().mkString(", ")}")
//    }
    ...
    saveEmbeddings("embeddings.csv", embeddings)
    println("Embeddings saved to embeddings.csv")

    def saveEmbeddings(filename: String, embeddings: INDArray): Unit = {
      val pw = new PrintWriter(filename)
      try {
        val rows = embeddings.rows()
        val cols = embeddings.columns()
        for (i <- 0 until rows) {
          val vector = embeddings.getRow(i).toDoubleVector
          val line = vector.mkString(",")
          pw.println(line)
        }
      } finally {
        pw.close()
